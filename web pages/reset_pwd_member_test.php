<?php
	...
	if(isset($_POST['submit']))
	{
		...
		if($old_pwd===$new_pwd)
		{
			$_SESSION['same_pwd_member']=1;
			header('location:reset_pwd_member.php');
		}
		else if($new_pwd===$confirm_pwd)
		{
			...
			if(mysqli_num_rows($result1)===1)
			{
				...
				if($result2)
				{
					$_SESSION['reset_pwd_member_success']=1;
					header('location:admin-view-unassigned.php');
				}
			}
			else
			{
				$_SESSION['reset_pwd_member_error2']=1;
				header('location:reset_pwd_member.php');
			}
		}
		else
		{
			$_SESSION['reset_pwd_member_error1']=1;
			header('location:reset_pwd_member.php');
		}
	}
	$conn->close();
