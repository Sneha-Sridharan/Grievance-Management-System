<?php
	...
	{	
		echo '<script type="text/javascript">window.onload = function(){alert("Comment added successfully")}</script>';
  ...
  <body>
    ...
    <!-------------SIDEBAR----------->
      ...
      <div id="content" class="p-4 p-md-5">
        ...
        <table>
          ...
          <tbody>
			<?php 
				...
				$sql="select * from assign_complaint where status IN('Assigned','In Progress') ORDER BY assigndate";
				$result=$conn->query($sql);
				while($row = $result->fetch_assoc()) {
					$id=$row['cid'];
					echo "<tr class='clickable-row' data-href='add-comment.php?id=$id' data-toggle='tooltip' data-placement='bottom' title='Click to add comments for this complaint'>";
					echo "<td>".$row['cid']."</td>";
					...
					$memid=$row['memid'];
					$sql3="select name from admin_member where memid='$memid'";
		...
		<br><h4 class="mb-4" style="margin-left: 115px;font-size:15px;color:#f8b739">Click on a complaint to add complaints</h4>
        </section>
      </div>
	</div>
