<?php
	session_start();
	if(isset($_SESSION['add_complaints_faculty_success']))
	{	
		echo '<script type="text/javascript">window.onload=function(){alert("Complaint registered successfully!")}</script>';
		unset($_SESSION['add_complaints_faculty_success']);
	}
	if(isset($_SESSION['view_complaints_faculty_error']))
	{	
		echo '<script type="text/javascript">window.onload=function(){alert("Error while viewing the complaints!")}</script>';
		unset($_SESSION['view_complaints_faculty_success']);
	}
	if(isset($_SESSION['reset_pwd_faculty_success'])) 
	{
		echo '<script type="text/javascript">window.onload=function(){alert("Password changed successfully!")}</script>';
		unset($_SESSION['reset_pwd_faculty_success']);
	}
  ...
  <body>
		...
		<div class="wrapper d-flex align-items-stretch">
	        ...
	        <ul class="list-unstyled components mb-5">
	          ...
	          <li>
              <a href="delete-complaints-faculty.php">Delete complaints</a>
	          </li>
			  <li>
          ...
          <nav id="thenav" class="navbar navbar-expand-lg navbar-light">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
             <ul class="list-unstyled components mb-5">
              ...
              <li>
              <a href="delete-complaints-faculty.php">DELETE</a>
              </li>
			  <li>
              <a href="reset_pwd_faculty.php">RESET PASSWORD</a>
              </li>
        ...
        <table>
          ...
          <tbody>
				<?php 
					...
					$sql1="select * from faculty_complaint where empno='$empno' ORDER BY cid DESC";
					...
					if($result1)
					{
						while($row1=$result1->fetch_assoc()) 
						{
							...
							{
								$_SESSION['view_complaints_faculty_error']=$error;
